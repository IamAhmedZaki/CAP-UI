...
const Shade = ({ selectedOptions = {}, onOptionChange }) => {
    ...
    const getDefaultShadeType = () => {
        return 'Blank'; // Default shade type
    };

    const getDefaultMaterialType = () => {
        return 'Uden kant'; // Default material
    };

    const getDefaultShadowTapeColor = () => {
        return 'INGEN'; // Default no shadow tape
    };

    // State variables with proper default values
    const [selectedShadeType, setSelectedShadeType] = useState(
        selectedOptions.Type || getDefaultShadeType()
    );
    ...
    const [selectedShadowTapeColor, setSelectedShadowTapeColor] = useState(
        selectedOptions.Skyggebånd || getDefaultShadowTapeColor()
    );
    const [engravingLine1, setEngravingLine1] = useState(
        selectedOptions['Skyggegravering Line 1'] || ''
    );
    const [engravingLine2, setEngravingLine2] = useState(
        selectedOptions['Skyggegravering Line 2'] || ''
    );
    const [engravingLine3, setEngravingLine3] = useState(
        selectedOptions['Skyggegravering Line 3'] || ''
    );
